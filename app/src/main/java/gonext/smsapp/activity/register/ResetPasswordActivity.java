    ...
    public void onClick(View v) {
       switch (v.getId()){
           case R.id.id_log_with:
               if(tvLogType.getText().toString().contentEquals("My user ID is an email")){
                   tvLogType.setText("My user ID is a mobile number");
                   llEmail.setVisibility(View.VISIBLE);
                   llMobile.setVisibility(View.GONE);
               }else{
                   tvLogType.setText("My user ID is an email");
